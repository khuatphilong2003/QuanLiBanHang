...
const ItemOrder = (props) => {
    ...
    const handleSelection = () => {
        if(quantity == 0){
            alert("Bạn phải nhập số lượng vào trước");
            return
        }
        if (!isSelected) {
            const objOrder = {
            ...
            listOrder.push(objOrder);
            console.log("Thêm order: " + listOrder.length);

        }
        else {
            const index = listOrder.findIndex(item => item.name === name);
            if (index > -1) {
                listOrder.splice(index, 1);
                console.log("xóa: " + listOrder.length);
            }
        }
    ...
    return (
        <View style={{
            ...
            <View style={{ flex: 1 }}>
                ...
                <View>
                    <Text style={{ fontSize: 30, color: 'red' }}>{price} vnd</Text>
                </View>
            </View>
            <View style={{ marginRight: 10 }}>
                ...
                <View>
                    ...
                    <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                        <TextInput placeholder='Nhập số' onChangeText={(Text) => setQuantity(Text)} keyboardType='number-pad'/>
                    </View>
                </View>
